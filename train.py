...
# default
RESUME_PATH = ''
# RESUME_PATH = 'data/results/models/recon/checkpoint_0300000.pth.tar'
...
# Fixed random number seed to ensure that the same input and 
# output are fixed every time the network is run
...
def train():
    ...
    for i in range(start_iter, args.num_iterations + 1):
        ...
        # soft render images
        render_images, laplacian_loss, tex_laplacian_loss, flatten_loss = model([images_a, images_b], 
                                                            [viewpoints_a, viewpoints_b],
                                                            task='train')
        laplacian_loss = laplacian_loss.mean()
        tex_laplacian_loss = tex_laplacian_loss.mean()
        l1_loss = torch.nn.L1Loss()
        col_l1_loss = l1_loss(render_images[0][0], images_a[-1])
        ...
        loss = multiview_iou_loss(render_images, images_a, images_b) + \
               ...
               args.lambda_laplacian * tex_laplacian_loss + \
               col_l1_loss + \
        ...
        if i % args.demo_freq == 0:
            ...
            demo_v, demo_f, textures = model.reconstruct(demo_image)
            textures = textures.clone().cuda().data.cpu().numpy()
            textures = np.ascontiguousarray(textures)
            textures = torch.from_numpy(textures)
            # print(textures[0])
            # print(textures.shape)   # torch.Size([1, 1280, 1, 3])
            # print(textures[0].shape)    # torch.Size([1280, 1, 3])
            # print(textures[0].shape[0]) # 1280
            srf.save_obj(demo_path, demo_v[0], demo_f[0], textures[0])
           
            imageio.imsave(os.path.join(image_output, '%07d_fake.png' % i), img_cvt(render_images[0][0]))
            imageio.imsave(os.path.join(image_output, '%07d_input.png' % i), img_cvt(images_a[-1]))
